...
% We consider a very simple neural network with only two neurons in one 
% layer. This is the equation of this layer
%
% S(Ax+b) = y
% 
% where S(v) = 1/(1+exp(-v)) is the sigmoid activation function computed element-wise
% A is 2xL weight matrix
% x is Lx1 input vector (FFT of the input signals)
% b is 2x1 bias vector
% y is 2x1 output vector

% We wish to have the following output:
% Dial tone 1: output y1 = [ 0   0 ]^T
% Dial tone 5: output y5 = [ 1   0 ]^T
% Dial tone 9: output y9 = [ 0   1 ]^T

% since the sigmoid function S(v): R --> ]0,1[, we will require the outputs
% to be around 0.9 and 0.1, so in practice we will admit

% Dial tone 1: output y1 = [ 0.1   0.1 ]^T
% Dial tone 5: output y5 = [ 0.9   0.1 ]^T
% Dial tone 9: output y9 = [ 0.1   0.9 ]^T
outputHigh=0.9;
outputlow=0.1;

y1= [outputlow*ones(Ntrain,1) outputlow*ones(Ntrain,1)];
y5= [outputHigh*ones(Ntrain,1) outputlow*ones(Ntrain,1)];
y9= [outputlow*ones(Ntrain,1) outputHigh*ones(Ntrain,1)];
...
%    E_{N x (L+1)}   *   P_{ L+1 x 2 } =  Y_tilde{ N x 2 }
